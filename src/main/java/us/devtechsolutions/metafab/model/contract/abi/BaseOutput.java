package us.devtechsolutions.metafab.model.contract.abi;

import org.jetbrains.annotations.ApiStatus;
import org.jetbrains.annotations.NotNull;
import org.jetbrains.annotations.UnknownNullability;

import java.util.List;
import java.util.stream.Collectors;

/**
 * @author LBuke (Teddeh)
 */
@ApiStatus.NonExtendable
public final class BaseOutput implements Output {

	private final String name;
	private final String type;
	private final String internalType;
	private final List<BaseComponent> components;

	public BaseOutput(@NotNull String name, @NotNull String type, @NotNull String internalType,
	                  @NotNull List<BaseComponent> components) {
		this.name = name;
		this.type = type;
		this.internalType = internalType;
		this.components = components;
	}

	/**
	 * Get the name of the output.
	 *
	 * @return output name
	 */
	@Override
	public @UnknownNullability String name() {
		return this.name;
	}

	/**
	 * Get the type of the output.
	 *
	 * @return output type
	 */
	@Override
	public @UnknownNullability String type() {
		return this.type;
	}

	/**
	 * Get the internal type of the output.
	 *
	 * @return output internal type
	 */
	@Override
	public @UnknownNullability String internalType() {
		return this.internalType;
	}

	/**
	 * Get a list of component for the output.
	 *
	 * @return output components
	 */
	@Override
	public @UnknownNullability List<Component> components() {
		return this.components.stream().map(c -> (Component) c).collect(Collectors.toList());
	}

	/** {@inheritDoc} */
	@Override
	public String toString() {
		return "BaseOutput{" +
				"name='" + name + '\'' +
				", type='" + type + '\'' +
				", internalType='" + internalType + '\'' +
				", components=" + components +
				'}';
	}
}
